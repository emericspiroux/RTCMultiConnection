'use strict'; // Last time updated: 2019-06-15 4:26:11 PM UTC
...
var RTCMultiConnection = function RTCMultiConnection(roomid, forceOptions) {
  ...
  function getUserMediaHandler(options) {
    if (currentUserMediaRequest.mutex === true && currentUserMediaRequest.queueRequests.length > 0) {
    ...
    if (currentUserMediaRequest.streams[idInstance]) {
      ...
      navigator.mediaDevices.getUserMedia(options.localMediaConstraints).then(function (stream) {
        stream.streamid = stream.streamid || stream.id || getRandomString();
        stream.idInstance = idInstance;
        if (options.session) stream.getTracks().forEach(function (track) {
          if (!options.session.audio && track.kind === "audio") track.enabled = false;
          if (!options.session.video && track.kind === "video") track.enabled = false;
        });
        streaming(stream);
      })["catch"](function (error) {
        options.onLocalMediaError(error, options.localMediaConstraints);
  ...
  (function (connection) {
    ...
    mPeer.onGettingRemoteMedia = function (stream, remoteUserId) {
      ...
      getRMCMediaElement(stream, /*#__PURE__*/function () {
        var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(mediaElement) {
          var sleep;
          return regeneratorRuntime.wrap(function _callee$(_context) {
            while (1) {
              switch (_context.prev = _context.next) {
                ...
                case 3:
                  if (connection.peers[remoteUserId].extra) {
                    _context.next = 8;
                    break;
                  }
